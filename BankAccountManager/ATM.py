...
class ATM(Account):
    ...
    def authenticated(self):
        if self.validateAccountNumber(self.userAccountNumber) and self.validatePIN(self.userPIN):
            authenticateQuery = "SELECT account_number FROM accounts WHERE account_number='{}' AND pin='{}'".format(self.userAccountNumber, self.hashPIN(self.userPIN))

            self.mycursor.execute(authenticateQuery)
